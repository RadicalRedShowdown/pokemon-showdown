export const Abilities: {[k: string]: ModdedAbilityData} = {
	...
	terashell: {
		...
		onEffectiveness(typeMod, target, type, move) {
			if (!target || target.species.name !== 'Terapagos') return;
	...
	jumpscare: {
		...
		onDamagingHit(damage, target, source, move) {
			if (target.illusion) {
				this.singleEvent('End', this.dex.abilities.get('jumpscare'), target.abilityState, target, source, move);
			}
		},
		onEnd(pokemon) {
			if (pokemon.illusion) {
				this.debug('jumpscare illusion cleared');
				pokemon.illusion = null;
				const details = pokemon.species.name + (pokemon.level === 100 ? '' : ', L' + pokemon.level) +
					(pokemon.gender === '' ? '' : ', ' + pokemon.gender) + (pokemon.set.shiny ? ', shiny' : '');
				this.add('replace', pokemon, details);
				this.add('-end', pokemon, 'Jump Scare');
				this.add('raw|<center><img src="https://tenor.com/ov3OPTanb3P.gif" height="96" alt="Jump Scare" /></center>');
		...
		shortDesc: "Illusion; this Pokemon's attacks deal 1.3x damage; shows a GIF when broken.",
	},
};
