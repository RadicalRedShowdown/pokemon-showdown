...
export class Pokemon {
	...
	constructor(set: string | AnyObject, side: Side) {
		...
		for (const moveid of this.set.moves) {
			...
			let basepp = move.maxpp ?? ((move.noPPBoosts || move.isZ) ? move.pp : move.pp * 8 / 5);
	...
	*/
	...
	formeChange(
		...
		if (isPermanent && (!source || !['disguise', 'iceface'].includes(source.id))) {
			...
			if ([
				'absolmega', 'houndoommega', 'slowbromega', 'gallademega', 'ampharosmega',
				'centiskorchseviimega', 'hornetagro',
			].includes(species.id)) {
				...
				} else if (toID(this.set.ability) === toID(baseSpecies.abilities['H']) && species.abilities['H']) {
